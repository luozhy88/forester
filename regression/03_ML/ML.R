...
# colnames(OTU_table_age)[1]<-"id"
# rownames(OTU_table_age)<-OTU_table_age$id
OTU_table_age[,"Age"]<-as.numeric(OTU_table_age[,"Age"])
...
# data(lisbon)
# View(lisbon)
project_name="shotgun_male_test8_"
target_name='Age'
df_train<-mbalanced_sample.male.train_otu
df_test<-mbalanced_sample.male.train_otu
df_train$Age<-df_train$Age %>% as.double() +0.00001
# check <- check_data(df_train, 'Age')

output2 <- forester::train(data = df_train,
                 y = 'Age',
                 bayes_iter = 0,
